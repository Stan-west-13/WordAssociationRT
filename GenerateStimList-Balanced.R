...
x <- map(c(1), function(x){
  ...
  df$condition <- rep(c("load","no_load"), each = 32)
  ...
  while(any(rle(df_split$load$TT)$lengths > 3) |any(rle(df_split$no_load$TT)$lengths > 3)){ ## This is saying while there are any runs > 3 in the condition column for a participant, keep running the for loop.
    for (i in 1:nrow(df_split$load)){
      df_split$load$TT[i] <- sample(trial_type_probs_L$TT, size = 1, prob = trial_type_probs_L$prob)
      df_split$load[i,2:9] <- slice_sample(combos[!combos %in% df_split$load[,2:9]], n = 1,replace = FALSE)
    ...
    for (i in 1:nrow(df_split$no_load)){
  ...
  write.csv(df_split$load, paste0("test_psychopy/TTA_",sprintf("%03d",unique(df_split$load$pp)),"_trial_list_L.csv" ),row.names = FALSE)
  write.csv(df_split$no_load, paste0("test_psychopy/TTA_",sprintf("%03d",unique(df_split$no_load$pp)),"_trial_list_NL.csv" ),row.names = FALSE)## Add N and NL to end of trial list for load and no load.
  return(df_split)
})
