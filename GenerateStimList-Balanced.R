...
system.time(with_progress({
    ...
    df_split$load$cue <- sample(c(sample(cues$high.co$cue,8),
                           sample(cues$low.co$cue,8),
                           sample(cues$high.nonco$cue,8),
                           sample(cues$low.nonco$cue,8)),32,replace = FALSE) ## sample 8 cues for load condition from each cue type.
    cues_nonselect <- map(cues, function(x){
      d <- x |>
        filter(!cue %in% df_split$load$cue)
      return(d)
    }) ## remove already sampled cues for no load condition
    df_split$no_load$cue <- sample(c(cues_nonselect$high.co$cue,
                              cues_nonselect$low.co$cue,
                              cues_nonselect$high.nonco$cue,
                              cues_nonselect$low.nonco$cue),32,replace = FALSE) ## populate no load cues
