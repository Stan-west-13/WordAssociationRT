---
title: "Creativity and Stereotypy Scores for Responses"
...
norms_list <- filter_participants_corrected %>%
  dplyr::select(condition, cue, response, revision, corrected_response) %>%
...
norms_list_corrected <- filter_participants %>%
  dplyr::select(condition, cue, response, revision) %>%
  filter(condition == 'peer') %>% 
  filter(response != "") %>%
  filter(response != "notsure", response != "idk") %>% 
  filter(!is.na(response)) %>% 
...
norm_dic <- norms_list %>% 
  dplyr::select(cue,response, frequency_response, max_response)

norm_dic_corrected <- norms_list %>% 
  dplyr::select(cue, corrected_response, frequency_corrected, max_corrected) %>% 
  distinct()
...
For the creative scores, a point is given if the response does not occur for that cue in the norms list. 
...
creative_list <- filter_participants_corrected %>% 
  dplyr::select(condition, participant, cue, response, corrected_response) %>% 
...
stereotypy_peer <- filter_participants %>%
  dplyr::select(participant, cue, response, condition) %>%
  filter(condition == 'peer') %>% 
  filter(response != "") %>%
  group_by(cue, response) %>% 
  mutate(frequency_response = n()) %>% 
  ...
  mutate(stereotypy_score = as.integer(
    response %in% response[frequency_response == max(frequency_response, na.rm = TRUE)])) %>% 
  dplyr::select(-condition, -frequency_response)
...
creative_all <- filter_participants_corrected %>% 
  ...
  mutate(creative_proportion_c = creative_count_corrected/60, .after = creative_count_corrected) %>% 
  ungroup() %>% 
  dplyr::select(c(-cue_onset:-key, -response.start, -response.stop))
...
creative_csonly <- creative_all %>% 
  dplyr::select(c(participant, condition, creative_count, creative_proportion)) %>% 
  distinct() 

creative_csonly_corrected <- creative_all %>% 
  dplyr::select(c(participant, condition, creative_count_corrected, creative_proportion_c)) %>% 
  distinct()
...
ggplot(creative_csonly_corrected, aes(x = reorder(condition, creative_count_corrected), y = creative_count_corrected, fill = condition)) + 
  stat_summary(aes(y = creative_count_corrected), fun = "mean", geom = "bar") +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = .1, linewidth = .75) + 
  theme_minimal() +
  scale_fill_manual(labels = c("peer", "child", "short", "creative") ,
                                    values = c("#f0b400","#AEAEAE", "#9C8BFF", "#4A248E"))+
  theme(plot.background = element_rect(fill = "#FCFBFF"))+
  theme(legend.position = 'none')

#AEAEAE #4A248E #9C8BFF
...
stereotypy_ssonly <- stereotypy_all %>% 
  dplyr::select(c(participant, condition, stereotypy_count, stereotypy_proportion)) %>% 
  distinct() 

stereotypy_ssonly_corrected <- stereotypy_all %>% 
  dplyr::select(c(participant, condition, stereotypy_count_corrected, stereotypy_proportion_c)) %>% 
  distinct()
...
ggplot(stereotypy_ssonly_corrected, aes(x = reorder(condition, -stereotypy_count_corrected), y = stereotypy_count_corrected, fill = condition)) + 
...
# Plot for average corrected stereotypy proportion for each condition
ggplot(stereotypy_ssonly_corrected, aes(x = condition, y = stereotypy_proportion_c, fill = condition)) + 
  stat_summary(aes(y = stereotypy_proportion_c), fun = "mean", geom = "bar") +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = .1, linewidth = .75)
...
### Plot for Psychonomics Poster
I am making a bar graph with proportion of stereotypy and creativity scores per condition. I want the stereotypy and creativity proportions for each condition next to each other

```{r}
## Histogram

w <- stereotypy_ssonly_corrected %>% 
  select(c('participant', 'condition', 'stereotypy_proportion_c'))
q <- creative_csonly_corrected %>% 
  select(c('participant', "condition", 'creative_proportion_c'))

histogram_means_wide <- merge(w, q)

histogram_means <- histogram_means_wide %>% 
  pivot_longer(
    cols = c('stereotypy_proportion_c', 'creative_proportion_c'),
    names_to = 'score',
    values_to = 'proportion',
    names_transform = list(score = as.factor)
  )

ggplot(histogram_means, aes(x = proportion, fill = score)) +
  geom_histogram(position = "identity", alpha = 0.6, bins = 40) + 
  geom_density(color = "black", alpha = 0.3) +
  facet_wrap(vars(condition)) +
  scale_fill_manual(values = c("#FDD023", "#613F9D")) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = "#FCFBFF"))

## Barplot

ggplot(histogram_means, aes(x = condition, y = proportion, fill = score)) + 
  stat_summary(aes(y = proportion), fun = "mean", geom = "bar", position = 'dodge') +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = .1, linewidth = .75, position = position_dodge(width = 0.9)) +
  scale_fill_manual(values = c("#FDD023", "#613F9D")) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = "#FCFBFF"))

ggsave(filename = 'task_adherence.png' ,width = 5, height = 4, dpi = 600, units = "in", device='png')

## ANOVA for graphs
s_c_model <- ezANOVA(data = histogram_means,
                    dv = proportion, 
                    within = score,
                    wid = participant,
                    between = condition, 
...
creative_se <- aov(proportion ~ score + Error(participant/score), 
                    data = histogram_means,
                    subset = condition == "creative")
summary(creative_se)

peer_se <- aov(proportion ~ score + Error(participant/score), 
                    data = histogram_means,
                    subset = condition == "peer")
summary(peer_se)

child_se <- aov(proportion ~ score + Error(participant/score), 
                    data = histogram_means,
                    subset = condition == "child")
summary(child_se)

short_se <- aov(proportion ~ score + Error(participant/score), 
                    data = histogram_means,
                    subset = condition == "short")
summary(short_se)

#######################
# New plots for poster 
######################

# Analysis for creative plot

## Variance among groups - failed to reject null that groups have equal variance p > .05
ggplot(creative_csonly_corrected, aes(x= condition, y = creative_count_corrected)) +
  geom_boxplot()

bartlett.test(creative_count_corrected ~ condition, data = creative_csonly_corrected)

## ezANOVA - sig difference between conditions; failed to reject null for levene's so equal variance
creative_model <- ezANOVA(
  data = creative_csonly_corrected,
  wid = participant,
  between = condition,
  dv = creative_count_corrected,
  detailed = T
...
c_model <- aov(creative_count_corrected ~ condition, data = creative_csonly_corrected)
summary(c_model)

# Normality is questionable 
shapiro.test(resid(c_model))

## Post-hoc - sig difference between creative-child and creative-peer
creative_csonly_corrected %>%
  emmeans_test(creative_count_corrected ~ condition, p.adjust.method = "bonferroni")

# Plot for creative count corrected
library(ggsignif)
plot_creative_c <- creative_csonly_corrected %>% 
  group_by(condition) %>% 
  summarize(mean = mean(creative_count_corrected),
            sd = sd(creative_count_corrected),
            n = length(creative_count_corrected),
            se = sd(creative_count_corrected)/sqrt(length(creative_count_corrected))) %>%
...
ggplot(plot_creative_c, aes(x = condition, y = mean, fill = condition)) + 
  geom_col()+
  geom_errorbar(aes(ymin = mean-se, ymax = mean+se), width = 0.1, linewidth = .75, size = 1)+
  theme_bw(base_size = 25) +
  coord_cartesian(ylim= c(0,30))+
  scale_fill_manual(labels = c("peer", "child", "short", "creative") ,
                                    values = c("#AEAEAE","#f0b400", "#9C8BFF", "#4A248E"))+
  theme(plot.background = element_rect(fill = "#FCFBFF"))+
  labs(y = "creative count",
       title = "Condition Creative Count"
       )+
  theme(legend.position = 'none',
        plot.title = element_text(hjust = 0.9)) +
  ggsignif::geom_signif(stat="signif",
                        position = "identity",
                        comparisons = list(c("child","creative"),
                                           c("peer","creative")),
                        annotations = "**",
                        textsize = 7,
                        tip_length = .1,
                        size = 1,
                        y_position = c(25.5,28))

ggsave(filename = 'creative_count_plot.png' ,width = 4.78, height = 7.5, dpi = 600, units = "in", device='png')


# Analysis for stereotypy scores

## Variance among groups - failed to reject null that groups have equal variance p > .05
ggplot(stereotypy_ssonly_corrected, aes(x= condition, y = stereotypy_count_corrected)) +
  geom_boxplot()

bartlett.test(stereotypy_count_corrected ~ condition, data = stereotypy_ssonly_corrected)

## ezANOVA - sig difference between conditions; failed to reject levene's null
stereotypy_model <- ezANOVA(
  data = stereotypy_ssonly_corrected,
  wid = participant,
  between = condition,
  dv = stereotypy_count_corrected,
  detailed = T
...
s_model <- aov(stereotypy_count_corrected ~ condition, data = stereotypy_ssonly_corrected)
summary(s_model)

# Normality is good
shapiro.test(resid(s_model))

## Post-hoc - sig difference 
stereotypy_ssonly_corrected %>%
  ungroup() %>% 
  emmeans_test(stereotypy_count_corrected ~ condition, p.adjust.method = "bonferroni")

# Plot for stereotypy
plot_stereotypy_c <- stereotypy_ssonly_corrected %>% 
  group_by(condition) %>% 
  summarize(mean = mean(stereotypy_count_corrected),
            sd = sd(stereotypy_count_corrected),
            n = length(stereotypy_count_corrected),
            se = sd(stereotypy_count_corrected)/sqrt(length(stereotypy_count_corrected))) %>%
  mutate(condition = factor(condition, 
                            levels = c("peer",
                                       "child",
                                       "short",
...
ggplot(plot_stereotypy_c, aes(x = condition, y = mean, fill = condition)) + 
  geom_col()+
  geom_errorbar(aes(ymin = mean-se, ymax = mean+se), width = 0.1, linewidth = .75, size = 1)+
  theme_bw(base_size = 25) +
  coord_cartesian(ylim= c(0,30))+
  scale_fill_manual(labels = c("peer", "child", "short", "creative") ,
                                    values = c("#AEAEAE","#f0b400", "#9C8BFF", "#4A248E"))+
  theme(plot.background = element_rect(fill = "#FCFBFF"))+
  labs(y = "stereotypy count",
       title = "Condition Stereotypy Count"
       )+
  theme(legend.position = 'none',
        plot.title = element_text(hjust = 0.9)) +
  ggsignif::geom_signif(stat="signif",
                        ...
                        tip_length = .1,
                        size = 1,
                        y_position = c(21.5,26,29))

ggsave(filename = 'stereotypy_count_plot.png' ,width = 4.78, height = 7.5, dpi = 600, units = "in", device='png')

# Making table for stereoptypy
library(apaTables)

apa.ezANOVA.table(stereotypy_model, filename = "stereotypy_anova_table.doc")
