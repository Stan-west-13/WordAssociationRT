...
#### 5/27/25 Edit:
I have added new columns to account for the revisions made. I have kept the original columns for comparision for how things have changed with the revisions. 
...
norm_dic_corrected <- norms_list %>% 
  select(cue, corrected_response, frequency_corrected) %>% 
  distinct()
...
creative_all <- filter_participants %>% 
  ...
  arrange(participant, cue, desc(frequency_response)) %>%
  group_by(participant, cue) %>% 
  mutate(creative_score = ifelse((response %in% norm_dic$response[cue == norm_dic$cue]), 0, ifelse(#assess if there is a revision for the response)),
         .after = response) %>% 
  mutate(c_score_peer = ifelse((response %in% norm_dic$response[cue == norm_dic$cue]) & (frequency_response == 1), 1, 0),
         .after = response) %>% 
  ungroup() %>% 
  # this adds the count of creative responses per participant
  group_by(participant) %>% 
  mutate(creative_count = ifelse(condition == "peer", sum(as.integer(c_score_peer)), sum(as.integer(creative_score))), .after = response) %>% 
...
"csonly" stands for creative score only
