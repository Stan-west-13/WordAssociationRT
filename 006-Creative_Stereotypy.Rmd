...
creative_all <- filter_participants_corrected %>% 
  ...
  mutate(c_score_peer = ifelse((response %in% norm_dic$response[cue == norm_dic$cue]) & (frequency_response == 1), 
                               1, 0),.after = response) %>%
  mutate(c_score_peer_corrected = ifelse((response %in% 
                                            norm_dic_corrected$corrected_response[cue == norm_dic_corrected$cue])
                                         & (frequency_corrected == 1), 1, 0), .after = corrected_response) %>% 
  ungroup() %>% 
