...
t <- stereotypy_ssonly_corrected %>% 
  group_by(condition) %>% 
  summarize(mean_proportion_s = mean(stereotypy_proportion_c)) 
  
t2 <- creative_csonly_corrected %>% 
  group_by(condition) %>% 
  summarize(mean_proportion_c = mean(creative_proportion_c))

score_means_wide <- merge(t, t2) 

score_means <- score_means_wide %>% 
  pivot_longer(
    cols = c('mean_proportion_s', 'mean_proportion_c'),
    names_to = 'score',
    values_to = 'proportion'
  )

ggplot(score_means, aes(x = condition, y = proportion, fill = score)) + 
